...
export function BtnCandidatar({ id }: { id: number }) {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `/api/vaga/candidato?id=${userId}&vagaId=${id}`
        );
  ...
  async function candidatar() {
    if (!isAdded) {
      try {
        const response = await fetch("/api/vaga/candidato", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ usuarioId: userId, vagaId: id }),
        });
  ...
  return (
    <div className="flex items-center justify-center md:justify-end">
      ...
      >
        {isAdded ? "Candidatado" : "Candidatar-se"}
      </button>
    </div>
  );
}
